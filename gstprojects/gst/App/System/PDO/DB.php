...
use App\System\PDO\Iterator as PDOIterator;
/*
 * PHP-PDO-MySQL-Class
...
class DB
{
	...
	public function __construct($table_name = "", $primary_key = "")
	{
		...
		$this->driver     = $config->get("db_driver");
...
// echo 'INSERT INTO ' . $tableName . ' (' . implode(',', $keys) . ')
// VALUES (:' . implode(',:', $keys) . ')',
// $params;
     ...
     */
    public function insertMulti( $params = array())
    {
		$tableName = $this->table_name;
        $rowCount = 0;
        if (!empty($params)) {
	...
	public function delete($where){
		$whereStr = null;
		$dbQuery = "DELETE FROM $this->table_name ";
		// where condition
		if (is_array($where)) {
			foreach ($where as $key => $value) {
				// Is there need to add "OR" condition?
			...
			$dbQuery .= " WHERE 1=1 {$whereStr}";
			$deleteParams =  array_values($where);
		} 
		$rowCount = $this->query($dbQuery, $deleteParams);
		return $rowCount;
	}
    /**
     * @return string
