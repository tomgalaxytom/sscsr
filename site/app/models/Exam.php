...
/**
 * 
 * Current Exam Checking
 * 
 */

                $result    = $this->select("em.exam_name, dtm.no_of_days,
                                            dbm.table_exam_year, 
                                            dbm.table_type, 
                                            dbm.table_name,
 ...
 foreach ($result as $row) {  // Foreach Block
     $enable_date_result    = $this->select("date1::date - INTEGER '$row->no_of_days' AS yesterday_date")
                                 ->from("$row->table_name")
                                 ->where(['tier_id' => "$row->tier_id"])
                                 ->order_by("id asc limit 1")
                                 ->get_list();
                                 //echo $this->last_query;
     $admitcard_enable_date = $enable_date_result[0]->yesterday_date;
 
     $exam_date_result      = $this->select("date1::date  AS exam_date")
                                 ->from("$row->table_name")
                                 ->where(['tier_id' => "$row->tier_id"])
                                 ->order_by("id asc limit 1")
                                 ->get_list();
 $current_exam_date         = $exam_date_result[0]->exam_date;
         ...
         if( $current_date >= $enable_date &&  $current_date <= $exam_date ){
             ...
             $this->updateRawQuery("public.sscsr_db_table_tier_master",$data, ['id' => $id]);
