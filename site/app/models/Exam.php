...
//          $sql = "select DISTINCT em.exam_name, dbm.table_exam_year, dbm.table_type, dbm.table_name,dbm.table_exam_short_name,dtm.tier_id as tier_id,tm.tier_name as tier_name,dtm.id as tableid   from exam_master em 
// join sscsr_db_table_master dbm on em.exam_short_name = dbm.table_exam_short_name and dbm.status='0'
// join sscsr_db_table_tier_master dtm on dbm.table_name = dtm.table_name
// join tier_master tm on cast(dtm.tier_id as char(255)) =  cast(tm.tier_id as char(255))   WHERE dtm.status='1' and  dbm.table_exam_short_name LIKE '%".$q."%'  order by  dbm.table_exam_year desc,dtm.tier_id asc "; 
