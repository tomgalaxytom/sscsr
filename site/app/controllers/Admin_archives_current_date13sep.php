...
class Admin extends BackEndController
{
    public $links = array(
        ...
        "archive_tender_link" => "admin/archive-tender/{id}",
    ...
    //     foreach ($data['nominations'] as $sn => $nomination) :
    //         $due_date = date('Y-m-d', strtotime($nomination->effort_from_date));
    //         $examname = $nomination->exam_name;
    //         $category_id = $nomination->category_id;
    //         $id = $nomination->nomination_id;

    //         if ($nomination->p_status == '1') {
    //             if (strtotime($due_date) < strtotime($date)) { // Date Comparison
    //                 $nomination_id = 0;
    //                 $nomination_data = [
    //                     'nomination_id' => $id,
    //                     'exam_name' => $examname,
    ...
    //                 $nomination =  new \App\Models\NominationArchieves();
    //                 if ($nomination_id == 0) { //nomination id is 0




    //                     if ($nomination->addNomination($nomination_data)) {  // Insert into Nomination Archieves

    //                         /***
    //                          * 
    //                          * Nomination Archieve Child insert
    //                          * 
    //                          */
    ...
    //                        //  print_r( $data['nominationchildlist']);
    //                        //  exit;



    //                         // echo  $nomination->nomination_id ;
    ...
    //                         foreach ($data['nominationchildlist'] as $i => $nominationchildarray) {


    //                             if ($lastinsertedid == $nominationchildarray->nomination_id) {
    //                                 //$nomination_id = $nominationchildarray->nomination_id;
    //                                 $pdf = $nominationchildarray->pdf_name;
    //                                 $attachment = $nominationchildarray->attachment;
    //                                 $nominationchild =  new \App\Models\NominationArchieveschild();
    ...
    //                                 $nominationchild->addNominationArchieveschild($nomination_child_data);

    //                                 // echo '<pre>';
    //                                 // print_r($nomination_child_data);
    //                                 // exit;
    //                             }
    //                         }
    ...
    //                         if ($nomination->deleteNomination($nominationchildarray->nomination_id)) {
    ...
    //                          * 
    //                          */
    //                     } // Insert into Nomination Archieves

    //                 } ////nomination id is 0

    //                 /**
    //                  * Delete Nomination based by id
    //                  * 
    //                  */
    ...
    // }
      ...
      //  echo '<pre>';
        ...
        $spCount = count((array)$data['selectionposts']);
        if($spCount > 0 ){ // selection Post  count if start
            foreach ($data['selectionposts'] as $sn => $selectionpost) :  // Foreach Start 
                $due_date = date('Y-m-d', strtotime($selectionpost->effort_from_date));

             

                
                $examname = $selectionpost->exam_name;
                $category_id = $selectionpost->category_id;
                $phase_id = $selectionpost->phase_id;
                $id = $selectionpost->selection_post_id;

                if($selectionpost->p_status == '1'){
                    if (strtotime($due_date) < strtotime($date)) { // Date Comparison
                        $selection_post_id = 0;
                        ...
                        $sparchives =  new \App\Models\SelectionpostArchives();
                        if ($selection_post_id == 0) { //nomination id is 0

                            if($sparchives->addSelectionpost($selectionpost_data)){   //insert selection Posts


                                $sparchives =  new \App\Models\SelectionpostArchives();

                                $lastinsertsql = $sparchives->lastInsertedId();
                                $lastinsertedid =  $lastinsertsql['max'];

                               // print_r(  $lastinsertedid);
                                $selectionpostchildclass = new Selectionpostschild();

                                $data['selectpostschildlist'] = $selectionpostchildclass->getSelectionpostschild();
                               ...
                               // print_R($data['selectpostschildlist'] );
                               // exit;

                                foreach ($data['selectpostschildlist'] as $i => $sparray) {

                                   // if ($lastinsertedid == $sparray->selection_post_child_id) {

                                        $pdf = $sparray->pdf_name;
                                        $attachment = $sparray->attachment;
                                        $spchild =  new \App\Models\SelectionpostschildArchives();
                                        ...
                                        $spchild->addSelectionpostchild($selectionpost_child_data);
                                ...
                                $sp =  new  \App\Models\Selectionpost();
                                if ($sp->deleteSelectionPost( $selectionpost->selection_post_id)) {
                                    $message = "Menu Deleted successfully";
                                    $message_type = "success";
                                }
                                // Delete Archives
            ...
            endforeach;                                            //  For Each End





        } // selection Post  count if End
         ...
         */
          ...
          */
          ...
          $date = date("Y-m-d");
          $noticesCount = count((array)$data['noticecreationlists']);
          if($noticesCount > 0 ){  // notices  count if start

            foreach ($data['noticecreationlists'] as $sn => $noticelist) :  // Foreach Start

                $due_date = date('Y-m-d', strtotime($noticelist->effect_from_date));
                $pdf_name = $noticelist->pdf_name;
                $attachment = $noticelist->attachment;
                $category_id = $noticelist->category_id;

                $id = date('Y-m-d', strtotime($noticelist->notice_id));
                if($noticelist->p_status == '1'){ // Status Check start if
                    ...
                    if (strtotime($due_date) < strtotime($date)) { // Date Comparison start if
                        ...
                        $noticesarchives =  new \App\Models\NoticeArchives();
                        if ($notice_id == 0) { //notice is 0

                            if($noticesarchives->addNotice($noticelist_data)){ // Insert Notice Archives Start 

                                $noticesarchives =  new \App\Models\NoticeArchives();
                               


                            }  // Insert Notice Archives End 

                            $noticesarchives =  new \App\Models\Notice();  

                            if($noticesarchives->deleteNoticeStatus( $noticelist->notice_id)){ //Delete  Notice Start 
         ...
         // $data['tendercreationlists'] = Helpers::getTenderListforAdmin();
...
public function ajaxresponseforNominationArchives()
{
     ...
     * 
     * 
     * Tender Archives By Month
     * 
     * 
     * 
     */
    public function ajaxresponseforTenderarchievesByMonth()
    {

        $year = $_POST['tender_year'];
        $month = $_POST['tender_month'];
        $elink = $_POST['elink'];
        $dlink = $_POST['dlink'];
        $alink = $_POST['alink'];
       ...
       // $model_archieves = $model->tenderArchievesByMonth($year, $month);
        $model_archieves = $model->tenderByMonth($year, $month,$effect_from_date ,$effect_to_date);

      //  echo '<pre>';
       // print_r($model_archieves);
        ...
        if(count( $arrayValue)>0){
            ...
            foreach ($arrayValue  as $row) {
				...
				$copy_tender_link_str = str_replace("{id}", $row['tender_id'], $this->links['copy_tender_link']);
...
$baseurl = $this->route->site_url("Admin/ajaxresponseforPublish");
...
if (@$_GET['status'] == 0 && $row['p_status'] != 1) {
	...
	style="" id="$tenderid" onclick="publishButton(
...
$actionoutput .= <<<TEXT
<input type="hidden" value="$tenderid " name="id" id="tender_id">
TEXT;
$actionoutput .= "</form>";
...
?>
              ...
              <?php  @$array['data'][] = array($row['tender_id'],$row['pdf_name'], $output, $row['effect_from_date'],$row['effect_to_date'],$flag,$actionoutput);
            }
            $string =    json_encode($array);
            $dataList = substr($string, 1, -1);
            echo "{" . str_replace('}', ']', str_replace('{', '[', $dataList)) . "}";
